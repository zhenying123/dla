...
#         self.logger.info(f'ROC AUC: {roc_test:.4f}, Test F1: {f1_test:.4f}, Test Parity: {parity_test:.4f}, Test Equality: {equality_test:.4f}')
    ...
    def generate_embeddings(self, data):
        with torch.no_grad():
            h = self.encoder(data.features, data.edge_index, data.adj_norm_sp)
        
        # test_mask = data.test_mask
        # test_indices = test_mask.nonzero(as_tuple=True)[0]
        # num_test_nodes = test_indices.shape[0]
        # selected_indices = np.random.choice(test_indices.cpu().numpy(), num_test_nodes , replace=False)
        num_nodes = h.size(0)

        # 随机生成节点索引
        selected_indices = np.random.choice(num_nodes, size=num_nodes, replace=False)

        # 使用这些索引选择节点
        selected_embeddings = h[selected_indices]

        # selected_embeddings = h[selected_indices]
        selected_sens = data.sens[selected_indices].cpu().numpy()
        selected_labels = data.labels[selected_indices].cpu().numpy()
        kpca = KernelPCA(n_components=2, kernel='rbf')
        embeddings_2d = kpca.fit_transform(selected_embeddings.cpu().numpy())
        # tsne = TSNE(n_components=2, random_state=0)
        # embeddings_2d = tsne.fit_transform(selected_embeddings.cpu().numpy())
        datasetname = self.args.dataset
        np.save(f'/home/yzhen/code/fair/FairSAD_copy/embeddings_{datasetname}.npy', embeddings_2d)
        np.save(f'/home/yzhen/code/fair/FairSAD_copy/labels_{datasetname}.npy', selected_labels)
        np.save(f'/home/yzhen/code/fair/FairSAD_copy/sens_{datasetname}.npy', selected_sens)
