...
def plot_embeddings_from_npy(ax, embedding_path, labels_path, sens_path, x_label='FairGNN'):
    ...
    labels = np.load(labels_path)
    sens = np.load(sens_path)
    num_nodes = embeddings_2d.shape[0]
    selected_indices = np.random.choice(num_nodes, size=(num_nodes) // 2, replace=False)
    embeddings_2d = embeddings_2d[selected_indices]
    labels = labels[selected_indices]
    sens = sens[selected_indices]
    ...
    for label in df['Label'].unique():
        for sens in df['Sens'].unique():
            ...
            ax.scatter(subset['X'], subset['Y'], label=f'Label {label}, Sens {sens}',
                       c=[colors[sens]], marker=shapes[label], edgecolor='k', s=200)
